...
Fixpoint decideOL_memo (fuel: nat) (g d: AnTerm) (cg cd: bool) (memo: MemoMap) : (bool * MemoMap) :=
  ...
  | None => (match fuel with
    ...
    | S n => let (b, m) :=
      ...
      | (N, L (Var a)) => mfalse
      | (N, R (Var a)) => mfalse
      | (L (Join a b), _) => decideOL_memo n (L a) d false cd &&& decideOL_memo n (L b) d false cd
      | (_, L (Join a b)) => decideOL_memo n g (L a) cg false &&& decideOL_memo n g (L b) cg false
      | (R (Meet a b), _) => decideOL_memo n (R a) d false cd &&& decideOL_memo n (R b) d false cd
      | (_, R (Meet a b)) => decideOL_memo n g (R a) cg false &&& decideOL_memo n g (R b) cg false
      ...
      | _ => (
        match g with 
        | L (Meet a b) => decideOL_memo n (L a) d false cd 
        | _ => mfalse
        end ||| (
        match g with 
        | L (Meet a b) => decideOL_memo n (L b) d false cd
        | _ => mfalse
        end ||| (
        match d with 
        | L (Meet a b) => decideOL_memo n g (L a) cg false 
        | _ => mfalse
        end ||| (
        match d with 
        | L (Meet a b) => decideOL_memo n g (L b) cg false 
        | _ => mfalse
        end ||| (
        match g with
        | R (Join a b) => decideOL_memo n (R a) d false cd
        | _ => mfalse
        end ||| (
        match g with
        | R (Join a b) => decideOL_memo n (R b) d false cd
        | _ => mfalse
        end ||| (
        match d with
        | R (Join a b) => decideOL_memo n g (R a) cg false 
        | _ => mfalse
        end ||| (
        match d with
        | R (Join a b) => decideOL_memo n g (R b) cg false
        | _ => mfalse
        end ||| (
        match (d, cg) with 
        | (N, false) => decideOL_memo n g g true true 
        | _ => mfalse
        end ||| (
        match (g, cd) with 
        | (N, false) => decideOL_memo n d d true true
        | _ => mfalse
        end  ||| (
        match d with 
        | N => mfalse
        | _ => decideOL_memo n g N cg true
        end ||| (
        match g with
        | N => mfalse
        | _ => decideOL_memo n N d true cd
        end
        ))))))))))))
      end) memo
    in (b, ((g, d, cg, cd), b) :: m) end)
end.


Definition decideOL_memo_simp (g d: AnTerm): bool := fst (decideOL_memo (anterm_size g * anterm_size d + 4)g d false false []).


Definition memomap_correct (l: MemoMap) :=  forall g d cg cd, 
  match find (g, d, cg, cd) l with
  | Some (_, true) => exists n,  (decideOL_opti n g d cg cd = true)
  | _ => True
  end.
...
Lemma memomap_correct_cons g d cg cd l e : 
  ...
  memomap_correct ((g, d, cg, cd, e) :: l) /\ (e = true -> exists n, decideOL_opti n g d cg cd = true).
Proof.
  intros. unfold memomap_correct. intros. split; intuition. 
  unfold find. fold find. destruct e.
  - destruct H1 as [n H1]; auto. destruct (memokey_eqdec (g0, d0, cg0, cd0) (fst (g, d, cg, cd, decideOL_opti n g d))).
    + rewrite memokey_eqdec_refl; simpl in *; auto. exists n. destruct (decideOL_opti n g d cg cd) eqn: res; simpl in *; congruence.
    + rewrite memokey_eqdec_refl2. 2: auto. apply H0.
  - destruct (memokey_eqdec (g0, d0, cg0, cd0) (fst (g, d, cg, cd, false))).
...
Lemma memomap_correct_false g d cg cd l: memomap_correct l -> memomap_correct ((g, d, cg, cd, false) :: l).
Proof.
  intros. unfold memomap_correct. intros. unfold find. fold find. destruct (memokey_eqdec (g0, d0, cg0, cd0) (g, d, cg, cd)).
  - rewrite memokey_eqdec_refl; auto.
...
Theorem decideOL_memo_correct : 
  ...
  (memomap_correct l) -> 
  (memomap_correct (snd (decideOL_memo n g d cg cd l))) /\
  (((fst (decideOL_memo n g d cg cd l)) = true) ->  exists n0, (decideOL_opti n0 g d cg cd) = true).
Proof.
  induction n.
  - intros. split.
    + intros. pose proof (H g d cg cd). simpl in *. destruct (find (g, d, cg, cd) l); simpl in *.
      * dest g; dest d; simpl in *; destruct p; simpl in *; auto.
      * auto.
    + specialize (H g d cg cd). simpl in *.
      destruct (find (g, d, cg, cd) l); intros.
  ...
  - intros.
    + simpl. pose proof H. unfold memomap_correct in H.  specialize (H g d cg cd).
      destruct (find (g, d, cg, cd) l) eqn: res; simpl in *. destruct p; simpl in *. auto. destruct b; simpl in *; auto. split; auto; intro; congruence.

      pose proof (memomap_correct_false g d cg cd l H0) as H1. 
      ...
      Ltac reduce_or rest IHn l H :=
        ...
        let b_ := (fresh "b") in let l_ := (fresh "l") in let found := (fresh "found") in
          lazymatch rest with
          | decideOL_memo ?n ?g ?d ?cg ?cd => 
            pose proof (IHn g d cg cd l H) as IHn_; simpl in *; destruct IHn_ as [IHn_snd IHn_fst];
            destruct (decideOL_memo n g d cg cd l) as [b_ l_] eqn: found;
          ...
          | decideOL_memo ?n ?g ?d ?cg ?cd ||| ?rest2 =>
            rewrite_mor (decideOL_memo n g d cg cd) rest2 l;
            pose proof (IHn g d cg cd l H) as IHn_; simpl in *; destruct IHn_ as [IHn_snd IHn_fst];
            destruct (decideOL_memo n g d cg cd l) as [b_ l_] eqn: found;
            destruct b_; simpl in *;
            only 1: (destruct IHn_fst as [n0 IHn_fst]; auto; simpl in *;
            ...
            idtac
          | (decideOL_memo ?n ?g1 ?d1 ?cg1 ?cd1) &&& (decideOL_memo ?n ?g2 ?d2  ?cg2 ?cd2) =>
            rewrite_mand (decideOL_memo n g1 d1 cg1 cd1) (decideOL_memo n g2 d2 cg2 cd2) l;
            ...
            destruct (decideOL_memo n g1 d1 cg1 cd1 l)  as [ b_ l_] eqn: found;
            pose proof (IHn g2 d2 cg2 cd2 l_ IHn_snd) as IHn_r; simpl in *; destruct IHn_r as [IHn_snd_r IHn_fst_r];
            destruct (decideOL_memo n g2 d2 cg2 cd2 l_)  as [ b_r l_r] eqn: found_r;
            simpl in *;
      ...
      | [ |- memomap_correct ( (?g, ?d, ?cg, ?cd, fst (?rest ?l)) :: snd (?rest ?l)) /\ _ ] => 
        apply memomap_correct_cons;
        reduce_or rest IHn l H0;
        idtac
...
Proof. 
  intros. assert (squash (OLProof (g, d))). (pose proof decideOL_memo_correct (anterm_size g * anterm_size d + 4) g d false false []). destruct H0. unfold memomap_correct; simpl in *; auto.
  specialize (H1 H). destruct H1 as [n0 H1]. apply decideOL_opti_correct with n0 false false; auto. 
